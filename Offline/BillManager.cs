...
namespace InfyPOS.Processors
{
    public class BillManager
    {
        ...
        public void LoadFromClientPackage(OfflineClient.ClientMasterPackage package)
        {
            ...
            System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "master.data"), package.master);
            ...
            {
                System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "stock.data"), package.stock);
            ...
            {
                System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "customer.data"), package.customer);
        ...
        public void PersistMasters()
        {
            ...
            System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "master.data"), masterdata);
        }
        ...
        public void PersistBill(bool clearcurrent)
        {
            if (clearcurrent)
            {
                ...
                System.IO.File.WriteAllBytes(System.IO.Path.Combine(BillManager.datadirectory, "offline.data"), offlinebytes);
            ...
            System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "bill.data"), masterdata);
        }
        ...
        internal void PersistSettlement(List<string> settled)
        {
            ...
            System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "settlement.data"), masterdata);
        }
        ...
        public void PersistMasters(OfflineClient.MasterData source)
        {
            ...
            System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "stock.data"), source.Stock);
            var masterdata = BufferedRealtimeCompressionEngine.Compress(System.Text.ASCIIEncoding.ASCII.GetBytes(Newtonsoft.Json.JsonConvert.SerializeObject(Data)));
            System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "master.data"), masterdata);
        }
        ...
        public bool Initialize(System.ComponentModel.DoWorkEventArgs e, System.ComponentModel.BackgroundWorker bgw)
        {
            try
            {
                ...
                if (System.IO.File.Exists(System.IO.Path.Combine(datadirectory, "master.data")))
                {
                    bgw.ReportProgress(5);
                    var masterdata = BufferedRealtimeCompressionEngine.Decompress(System.IO.File.ReadAllBytes(System.IO.Path.Combine(datadirectory, "master.data")));
                    Data = Newtonsoft.Json.JsonConvert.DeserializeObject<BillSourceData>(System.Text.ASCIIEncoding.ASCII.GetString(masterdata));
                    bgw.ReportProgress(25);
                    if (System.IO.File.Exists(System.IO.Path.Combine(datadirectory, "stock.data")))
                    {
                        bgw.ReportProgress(30);
                        Stocks = Processors.DataTableCustomFormatter.Deserialize(System.IO.File.ReadAllBytes(System.IO.Path.Combine(datadirectory, "stock.data")), true);
                        Stocks.CaseSensitive = true;
                        //Stocks.PrimaryKey = new System.Data.DataColumn[] { Stocks.Columns["barcode"] };
                        bgw.ReportProgress(50);
                        if (System.IO.File.Exists(System.IO.Path.Combine(datadirectory, "bill.data")))
                        {
                            bgw.ReportProgress(55);
                            var billdata = BufferedRealtimeCompressionEngine.Decompress(System.IO.File.ReadAllBytes(System.IO.Path.Combine(datadirectory, "bill.data")));
                        ...
                        if (System.IO.File.Exists(System.IO.Path.Combine(datadirectory, "settlement.data")))
                        {
                            bgw.ReportProgress(80);
                            var billdata = BufferedRealtimeCompressionEngine.Decompress(System.IO.File.ReadAllBytes(System.IO.Path.Combine(datadirectory, "settlement.data")));
        ...
        public class CustomerManager
        {
            ...
            {
                if (System.IO.File.Exists(System.IO.Path.Combine(datadirectory, "customer.data")))
                {
                    var customerdata = BufferedRealtimeCompressionEngine.Decompress(System.IO.File.ReadAllBytes(System.IO.Path.Combine(datadirectory, "customer.data")));
            ...
            public void Persist()
            {
                ...
                System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "customer.data"), masterdata);
            }
        }
        public class NoSequenceManager
        {
            ...
            public void Inialize()
            {
                ...
                if (System.IO.File.Exists(System.IO.Path.Combine(datadirectory, "sequence.data")))
                {
                    var billdata = BufferedRealtimeCompressionEngine.Decompress(System.IO.File.ReadAllBytes(System.IO.Path.Combine(datadirectory, "bill.data")));
                    list = Newtonsoft.Json.JsonConvert.DeserializeObject<List<NoSequence>>(System.Text.ASCIIEncoding.ASCII.GetString(billdata));
                }
                else
                {
            ...
            public void Persist()
            {
                ...
                var masterdata = BufferedRealtimeCompressionEngine.Compress(System.Text.ASCIIEncoding.ASCII.GetBytes(Newtonsoft.Json.JsonConvert.SerializeObject(list)));
                System.IO.File.WriteAllBytes(System.IO.Path.Combine(datadirectory, "sequence.data"), masterdata);
            }
